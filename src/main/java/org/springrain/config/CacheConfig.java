...
public class CacheConfig {
	 ...
	 */
	/*
	 * @Bean("cacheManager") public CacheManager cacheManager() {
	 * 
	 * FrameRedissonSpringCacheManager cacheManager= new
	 * FrameRedissonSpringCacheManager(redissonClient());
	 * cacheManager.setCacheTimeOut(cacheTimeOut);
	 * 
	 * return cacheManager; }
	 * 
	 * @Bean("redissonClient") public RedissonClient redissonClient() {
	 * 
	 * // 连接超时时间 int connectTimeOut = 10000; // 重试次数 int retryAttempts = 6;
	 * 
	 * if (StringUtils.isBlank(redisHostPort)) { return null; }
	 * redisHostPort=redisHostPort.trim();
	 * 
	 * Config config = new Config(); String[] ipports = redisHostPort.split(",");
	 * 
	 ...
	 * 
	 * if(StringUtils.isNotBlank(password)) { useSingleServer.setPassword(password);
	 * }
	 * 
	 * 
	 * 
	 * } else {// redis 集群.默认读slave ClusterServersConfig useClusterServers =
	 ...
	 * .setSlaveConnectionPoolSize(maxActive).setMasterConnectionMinimumIdleSize(
	 * minIdle)
	 * .setSlaveConnectionMinimumIdleSize(minIdle).setReadMode(ReadMode.SLAVE).
	 ...
	 * 
	 * 
	 * } // JDK的序列化 // config.setCodec(new SerializationCodec());
	 * 
	 * //fst序列化 config.setCodec(new FstCodec());
	 * 
	 ...
	 * 
	 * @Bean("redisOperation") public RedisOperation redisOperation() {
	 * RedisOperation redisOperation=new RedisOperation();
	 * redisOperation.setRedissonClient(redissonClient()); return redisOperation; }
	 */
