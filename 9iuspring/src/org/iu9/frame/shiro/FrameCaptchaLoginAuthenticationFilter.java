...
@Component("frameCaptchaLogin")
public class FrameCaptchaLoginAuthenticationFilter  extends FormAuthenticationFilter{

	
	//验证码参数名称
    private String captchaParam = GlobalStatic.DEFAULT_CAPTCHA_PARAM;
    //在session中的存储验证码的key名称
    private String sessionCaptchaKeyAttribute = GlobalStatic.DEFAULT_CAPTCHA_PARAM;
    //在session中存储的登录次数名称
    private String loginNumKeyAttribute = GlobalStatic.DEFAULT_LOGIN_NUM_KEY_ATTRIBUTE;
    //登录次数超出allowLoginNum时，存储在session记录是否展示验证码的key名称
    private String sessionShowCaptchaKeyAttribute = GlobalStatic.DEFAULT_SHOW_CAPTCHA_KEY_ATTRIBUTE;
    //允许登录次数，当登录次数大于该数值时，会在页面中显示验证码
    private Integer allowLoginNum = GlobalStatic.allowLoginNum;
	...
	protected boolean executeLogin(ServletRequest request,ServletResponse response) throws Exception {
		...
		if (number > getAllowLoginNum()) {
			...
			//如果验证码不匹配，登录失败
			if (StringUtils.isBlank(submitCaptcha) ||StringUtils.isBlank(currentCaptcha)||!currentCaptcha.toLowerCase().equals(submitCaptcha.toLowerCase())) {
